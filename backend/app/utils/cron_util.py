...
class CronUtil:
    ...
    def __valid_sum(
        ...
        if match:
            start, end = int(match.group(1)), int(match.group(2))
            return start_range_a <= start <= start_range_b and end_range_a <= end <= end_range_b and start + end <= sum_range
        return False
    ...
    def validate_year(cls, year: str) -> bool:
        ...
        return bool(
            ...
            or ((len(year) == 4 or "," in year) and all(int(item) in future_years and current_year <= int(item) <= 2099 for item in year.split(","))),
        )
    ...
    def validate_cron_expression(cls, cron_expression: str) -> bool | None:
        ...
        validation = second_validation and minute_validation and hour_validation and day_validation and month_validation and week_validation
        if len(values) == 6:
            return validation
        if len(values) == 7:
